...
ipcMain.handle('set-file-path', async (event, filePath) => {
  // Directly set a file path without opening dialog
  // Used for recent files
...
ipcMain.handle('set-project-path', async (event, projectPath) => {
  // Directly set a project path without opening dialog
  // Used for recent files
...
ipcMain.handle('export-explanation', async (event, content, defaultFileName) => {
  try {
    const result = await dialog.showSaveDialog(mainWindow, {
      title: 'Export Explanation',
      defaultPath: defaultFileName || 'explanation.md',
      filters: [
...
ipcMain.handle('add-recent-file', (event, filePath, type) => {
  try {
    const recent = store.get('recentFiles', []);
    // Remove if already exists
    const filtered = recent.filter(item => !(item.path === filePath && item.type === type));
    // Add to beginning
    filtered.unshift({ path: filePath, type, timestamp: Date.now() });
    // Keep only last 20
