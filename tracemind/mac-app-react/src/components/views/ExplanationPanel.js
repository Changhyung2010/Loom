...
function ExplanationPanel({ explanation, loading = false, filePath, projectPath, apiKey, model, analysisMode, githubUrl, githubData, onToggleFullscreen, isFullscreen, projectStats }) {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'f' && viewMode === 'text' && explanation) {
        e.preventDefault();
        setShowSearch(true);
  ...
  }, [viewMode, explanation, showSearch]);
    ...
    <div className="explanation-panel" style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          {/* Search button - only show in text view */}
          {viewMode === 'text' && explanation && (
            <>
              {showSearch ? (
            ...
            <button
              onClick={async () => {
                if (window.electronAPI && window.electronAPI.copyToClipboard) {
                  const result = await window.electronAPI.copyToClipboard(explanation);
                  if (result.success) {
                    setCopySuccess(true);
              ...
              style={{
                ...
                borderRadius: '6px',
                padding: '6px 10px',
                color: copySuccess ? COLORS.accent : COLORS.textSecondary,
                fontSize: '12px',
                cursor: 'pointer',
                transition: 'all 0.15s ease',
                display: 'flex',
          ...
          {/* Export button - only show when there's content */}
          {viewMode === 'text' && explanation && (
            <button
              onClick={async () => {
                if (window.electronAPI && window.electronAPI.exportExplanation) {
                  const fileName = filePath 
                    ? `${filePath.split('/').pop().replace(/\.[^/.]+$/, '')}_explanation.md`
                    : projectPath
                    ? `${projectPath.split('/').pop()}_explanation.md`
                    : 'explanation.md';
                  
                  const result = await window.electronAPI.exportExplanation(explanation, fileName);
                  if (result.success) {
              ...
              style={{
                ...
                borderRadius: '6px',
                padding: '6px 10px',
                color: exportSuccess ? COLORS.accent : COLORS.textSecondary,
                fontSize: '12px',
                cursor: 'pointer',
                transition: 'all 0.15s ease',
                display: 'flex',
      ...
      {viewMode === 'text' ? (
        ...
        >
          {!explanation ? (
            ...
            <div className="explanation-content" style={{
              ...
              {searchQuery && (
                <div style={{
                  ...
                  <span>
                    {(() => {
                      const matches = (explanation.match(new RegExp(searchQuery, 'gi')) || []).length;
            ...
            text: ({node, ...props}) => {
              if (!searchQuery) return <span {...props} />;
